...
`sigil` is a provider for the [freedesktop.org Secret Service API](https://specifications.freedesktop.org/secret-service/latest/). This specification defines a standard D-Bus interface for securely storing and retrieving passwords and other secrets on Linux systems.
...
`sigil` maps its internal state to the D-Bus object paths defined by the spec:
...
4.  The daemon spawns an external UI (like `sigil-prompter`) to ask the user for the master password.
...
3.  **Key Derivation**: `sigil` and the client both use **HKDF-SHA256** (with no salt and empty info) to derive a 128-bit AES key from the DH shared secret. *(Note: The spec text says "SHA256", but the canonical `libsecret` implementation uses HKDF. Interoperability requires HKDF).*
