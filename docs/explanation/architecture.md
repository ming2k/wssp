...
login + screensaver integration. See [../how-to/configure-unlock.md](../how-to/configure-unlock.md).
...
The daemon is launched on demand by the session bus, not by the user. There are two
mechanisms a process can register to claim a well-known D-Bus name like
`org.freedesktop.secrets`, and they are mutually exclusive.

### Mechanism A: dbus-daemon `.service` activation

A file under `~/.local/share/dbus-1/services/` or `/usr/share/dbus-1/services/` of the form:

```ini
[D-BUS Service]
Name=org.freedesktop.secrets
Exec=/path/to/some-daemon
```

tells `dbus-daemon` (or `dbus-broker`): when any client makes a method call to a name with
no current owner, scan these files, find a matching `Name=`, and fork-exec the `Exec=`
binary. The launched process then claims the name and answers the pending call. This is
the classic D-Bus auto-start mechanism and is how gnome-keyring, kwallet, and most
desktop services have historically been activated.

### Mechanism B: systemd `Type=dbus` activation (WSSP)

A systemd user unit with:

```ini
[Service]
Type=dbus
BusName=org.freedesktop.secrets
ExecStart=/usr/bin/wssp-daemon
```

registers the bus-name claim with systemd directly. When a client calls the name, systemd
starts the unit, the unit claims the name, and the call proceeds. systemd manages restarts,
logging, sandboxing, and dependency ordering — none of which a bare `Exec=` activation file
provides.

WSSP ships only the systemd unit. It deliberately does **not** ship a
`/usr/share/dbus-1/services/org.freedesktop.secrets.service` file, because:

1. **Sandbox enforcement.** The unit applies `ProtectSystem=full`, `PrivateTmp=yes`,
   `MemoryDenyWriteExecute=yes`, and other hardening directives that bare `.service`
   activation cannot express.
2. **One-owner invariant.** systemd refuses to load two units claiming the same `BusName=`,
   which catches packaging mistakes loudly instead of silently racing two daemons.
3. **PAM and inotify hooks.** The daemon needs `PassEnvironment=` for `WAYLAND_DISPLAY` and
   `ReadWritePaths=` for the vault directory; these are unit-level concerns.

### Why both at once breaks

If a `.service` activation file *and* a `Type=dbus` systemd unit both declare
`org.freedesktop.secrets`, the bus daemon and systemd race. systemd refuses to load the
unit ("Two services allocated for the same bus name … refusing operation") and `dbus-daemon`
fork-execs whatever the `.service` file points at — typically a stale dev binary. The
machine then runs a daemon nobody installed, with none of the sandbox guarantees.

The cleanup procedure for this is in
