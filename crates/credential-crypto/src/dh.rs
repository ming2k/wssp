...
impl DhSession {
    pub fn generate() -> Result<Self> {
        let prime = BigUint::parse_bytes(DH_PRIME_HEX.as_bytes(), 16)
            .ok_or_else(|| CredentialError::CryptoFailure("Invalid DH prime hex".into()))?;
    ...
    pub fn derive_shared_key(&self, peer_public_bytes: &[u8]) -> Result<[u8; 16]> {
        ...
        if peer_public <= BigUint::from(1u32) || peer_public >= prime.clone() - 1u32 {
            return Err(CredentialError::CryptoFailure("Invalid peer DH public key".into()));
        }
        ...
        hk.expand(&[], &mut aes_key)
            .map_err(|e| CredentialError::CryptoFailure(format!("HKDF expand failed: {e}")))?;
...
pub fn encrypt_secret_service_aes128(
    ...
    let cipher = Aes128CbcEnc::new_from_slices(aes_key, &iv)
        .map_err(|e| CredentialError::CryptoFailure(format!("AES-128-CBC init failed: {e}")))?;

    let mut buf = vec![0u8; plaintext.len() + 16];
    buf[..plaintext.len()].copy_from_slice(plaintext);
    let ciphertext = cipher
        .encrypt_padded_mut::<Pkcs7>(&mut buf, plaintext.len())
        .map_err(|e| CredentialError::CryptoFailure(format!("AES-128-CBC encryption failed: {e}")))?
        .to_vec();
...
pub fn decrypt_secret_service_aes128(
    ...
    if iv.len() != 16 {
        return Err(CredentialError::CryptoFailure("IV must be 16 bytes".into()));
    }
    ...
    let decrypted = cipher
        ...
        .map_err(|e| CredentialError::CryptoFailure(format!("AES-128-CBC decryption failed: {e}")))?;
