...
pub fn encrypt_xchacha20poly1305(
    ...
    let cipher = XChaCha20Poly1305::new_from_slice(key.as_bytes())
        .map_err(|e| CredentialError::CryptoFailure(format!("Failed to initialize cipher: {e}")))?;
    ...
    let ciphertext = cipher
        ...
        .map_err(|e| CredentialError::CryptoFailure(format!("Encryption failed: {e}")))?;
...
pub fn decrypt_xchacha20poly1305(
    ...
    if blob.len() < NONCE_LEN + 16 {
        return Err(CredentialError::CorruptData(format!(
    ...
    let decrypted = cipher
        ...
        .map_err(|_| CredentialError::CryptoFailure("Decryption failed / MAC verification failed".into()))?;
