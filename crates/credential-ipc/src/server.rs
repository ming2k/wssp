...
use credential_core::{CredentialError, Namespace, Purpose, Result, Subject};
...
impl NativeIpcServer {
    pub fn new(socket_path: PathBuf, service: CredentialService) -> Self {
        Self {
            socket_path,
            service,
        }
...
async fn handle_connection(mut stream: UnixStream, service: CredentialService) -> Result<()> {
    ...
    loop {
        let req = match read_request(&mut stream).await {
            Ok(r) => r,
            Err(CredentialError::Io(ref e)) if e.kind() == std::io::ErrorKind::UnexpectedEof => {
                break;
            }
            Err(e) => return Err(e),
        };
        ...
        let resp = match req {
            ...
            IpcRequest::GetApplicationSecret {
                    ...
                    .await
                {
                    ...
                    Err(CredentialError::Locked) => IpcResponse::Locked,
                    Err(e) => IpcResponse::Error(e.to_string()),
                }
            }
        };
