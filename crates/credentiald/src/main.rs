...
use credential_ipc::NativeIpcServer;
use credential_secret_service::{Collection, Prompt, SecretServiceDbus};
use credential_service::CredentialService;
use credential_store::{ensure_secure_dir, FileVaultStore, StoredVaultData};
...
fn get_data_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("CREDENTIALD_DATA_DIR") {
...
fn get_runtime_socket_path() -> PathBuf {
    if let Ok(path) = std::env::var("CREDENTIALD_SOCKET_PATH") {
        return PathBuf::from(path);
    }
    let runtime_dir = std::env::var("XDG_RUNTIME_DIR").unwrap_or_else(|_| "/tmp".into());
    PathBuf::from(runtime_dir).join("credentiald/native.sock")
}

#[tokio::main]
async fn main() -> Result<(), Box<dyn Error>> {
    tracing_subscriber::fmt::init();
    info!("Starting credentiald daemon...");
    ...
    info!("credentiald successfully registered on D-Bus as org.freedesktop.secrets.");
