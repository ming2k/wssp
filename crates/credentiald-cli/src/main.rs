use credential_crypto::{
    decode_kdf, derive_key_argon2id, encode_kdf, generate_salt, KdfParams, MasterKey,
    DEFAULT_SALT_LEN,
};
use credential_store::{atomic_replace, FileVaultStore, StoredVaultData};
...
fn get_data_dir() -> PathBuf {
    if let Ok(dir) = env::var("CREDENTIALD_DATA_DIR") {
        return PathBuf::from(dir);
    }
    if let Some(proj_dirs) = ProjectDirs::from("org", "freedesktop", "credentiald") {
        proj_dirs.data_dir().to_path_buf()
    } else {
        PathBuf::from(".local/share/credentiald")
    }
}
...
fn init(no_password: bool) {
    ...
    if store.exists() || paths.key.exists() {
        ...
        eprintln!("Use 'credentiald-cli reset' first if you want to start over.");
        std::process::exit(1);
    }
...
fn change_password() {
    ...
    if !store.exists() {
        eprintln!("No vault found. Run 'credentiald-cli init' first.");
        std::process::exit(1);
    }
...
fn reset() {
    ...
    if deleted {
        println!("Vault reset. Run 'credentiald-cli init' to create a new one.");
    } else {
...
fn usage() {
    eprintln!("Usage:");
    eprintln!("  credentiald-cli init                 # first-time setup with password (prompted)");
    eprintln!("  credentiald-cli init --no-password   # first-time setup without password (requires FDE)");
    eprintln!("  credentiald-cli change-password      # change password on existing vault");
    eprintln!("  credentiald-cli reset                # delete vault and start over");
}
