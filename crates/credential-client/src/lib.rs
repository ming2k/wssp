...
pub use client::{CredentialClient, DEFAULT_SOCKET_SUBPATH};
...
mod tests {
    ...
    async fn test_client_integration() {
        let temp_dir = std::env::temp_dir().join(format!("credentiald_client_test_{}", std::process::id()));
        ...
        let client = CredentialClient::new(sock_path);
