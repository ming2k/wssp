...
pub use state::{CollectionRecord, CredentialService, ItemRecord};

#[cfg(test)]
mod tests {
    use super::*;
    use credential_core::{Namespace, Purpose, Subject};
    ...
    async fn test_service_lifecycle_and_crud() {
        let temp_dir = std::env::temp_dir().join(format!("credentiald_svc_test_{}", std::process::id()));
